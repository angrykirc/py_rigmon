...
def get_all_rigs() -> list:
    return rigs.values()

def check_rig_exists(name: str) -> bool:
    if name in rigs:
        return True
    return False

# Registers a new, empty Rig, and returns a reference to it to be filled with data
def add_new_rig(name: str) -> monitor_rig_data:
    r = monitor_rig_data()
    rigs[name] = r
    return r
    
# Returns a Rig class by its hostname, or None if not present
def get_rig_by_name(name: str) -> monitor_rig_data:
    if not name in rigs:
        return None
        
    return rigs[name]
    
def build_response(rig: monitor_rig_data, num: int) -> dict:
    s = dict()
    s["result"] = num
    c = rig.config
    if rig.config_ver == 0:
...
def get_test_config() -> stored_rig_data:
...
def process_request(m: monitor_rig_data, j: dict):
    # parse overall stats
